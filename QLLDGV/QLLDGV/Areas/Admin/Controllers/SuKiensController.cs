...
{
    public class SuKiensController : BaseController
    {
        ...
        public ActionResult Index(int? page, string SearchString, string currentFilter)
        {
            ...
            {
                links = db.SuKiens.Where(n => n.TenSuKien.Contains(SearchString) ||
                                            n.MonHoc.TenMH.Contains(SearchString) ||
                                            n.HocPhan.TenKhoa.Contains(SearchString) ||
                                            n.GIAOVIEN1.HoTenGV.Contains(SearchString)).ToList();
            }
            else
            {
                links = db.SuKiens.Include(l => l.GIAOVIEN1).Include(l => l.MonHoc).Include(l => l.HocPhan).ToList();
            }
            ViewBag.CurrentFilter = SearchString;
            int pageSize = 5;
            int pageNumber = (page ?? 1);
            links = links.OrderByDescending(n => n.GIAOVIEN1.HoTenGV).ToList();
        ...
        {
            ViewBag.GiaoVien = new SelectList(db.GIAOVIENs, "MaGV", "HoTenGV");
            ViewBag.Khoa = new SelectList(db.Khoas, "MaKhoa", "MonHoc");
        ...
        public ActionResult Create([Bind(Include = "MaSuKien,TenHP,Khoa,GiaoVien,TenSuKien,StartDate")] SuKien suKien)
        {
        ...
        public JsonResult GetGVList(string MaHP)
        {
            db.Configuration.ProxyCreationEnabled = false;
            List<GIAOVIEN> GiaoVienList = db.GIAOVIENs.Where(x => x.BoMonPhuTrach == MaHP).ToList();
        ...
        public ActionResult Edit(Guid? id)
        {
            ...
            ViewBag.GiaoVien = new SelectList(db.GIAOVIENs, "MaGV", "HoTenGV", suKien.GiaoVien);
            ViewBag.Khoa = new SelectList(db.Khoas, "MaKhoa", "TenKhoa", suKien.Nhom);
            ViewBag.TenHP = new SelectList(db.MonHocs, "MaMH", "TenMH", suKien.TenHP);
            return View(suKien);
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "MaSuKien,TenHP,Khoa,GiaoVien,TenSuKien,StartDate")] SuKien suKien)
        {
            if (ModelState.IsValid)
            {
